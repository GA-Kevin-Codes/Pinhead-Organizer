...
const AUTH_MESSAGE_KEY = "pinhead-basic-auth-message:v1";
...
function bindEvents() {
  els.loginButton.addEventListener("click", () => {
    state.authMessage = "Redirecting to Wikimedia...";
    renderLoginStatus();
...
async function finishOauthCallbackIfNeeded() {
  ...
  if (error) {
    ...
    state.authMessage = `Login failed: ${error}`;
    renderLoginStatus();
  ...
  if (!stored) {
    history.replaceState({}, "", currentPageUrl());
    state.authMessage = "Login failed: callback arrived without saved PKCE state.";
    renderLoginStatus();
  ...
  if (pkce.state !== returnedState) {
    history.replaceState({}, "", currentPageUrl());
    state.authMessage = "Login failed: callback state did not match.";
    renderLoginStatus();
  ...
  try {
    ...
    if (!tokenResponse.ok || !tokenData.access_token) {
      ...
      clearAuth();
      state.authMessage = `Login failed: ${message}`;
      history.replaceState({}, "", currentPageUrl());
...
function saveAuthMessage() {
  if (state.authMessage) {
    localStorage.setItem(AUTH_MESSAGE_KEY, state.authMessage);
  } else {
